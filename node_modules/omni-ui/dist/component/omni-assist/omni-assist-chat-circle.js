...
class OmniAssistChatCircle extends OmniElement {
  ...
  async connectedCallback() {
    ...
    if (this.genAiModels?.length && !this.selectedGenAiModel) {
      this.selectedGenAiModel = this.genAiModels[0];
      this.dispatchNewEvent("gen-ai-model-change", { detail: this.selectedGenAiModel });
    }
  }
  #handleDragend;
  willUpdate(changed) {
      ...
      <omni-style>
        ...
        <omni-tile
          ...
          <omni-toolbar slot="header" class="p-3">
            <div slot="start" class="title-and-model">
              <h3 class="title m-0 p-0 mr-4">Assist</h3>
              ${this.genAiModels?.length > 0 ? html`
                    <omni-dropdown
                      variant="label"
                      .options=${this.genAiModels}
                      .value=${[this.selectedGenAiModel]}
                      @change=${(e) => {
      e.stopPropagation();
      if (!e.detail) {
        e.target.select([this.selectedGenAiModel]);
        return;
      }
      this.selectedGenAiModel = e.detail;
      this.dispatchNewEvent("gen-ai-model-change", { detail: e.detail });
    }}>
                    </omni-dropdown>
                  ` : nothing}
            </div>
                  ...
                  ${item.role === "assistant" ? html`
                        ...
                        <slot name="response-footer">
                          <omni-assist-feedback-controls
                            ...
                            ?unhide=${this.unhide && !this.isStreaming && !this.isLoading}
                            ?showCopy=${this.showCopy && !this.isStreaming && !this.isLoading}
                            .response=${item}
...
__decorateClass([
  property({ type: Boolean })
], OmniAssistChatCircle.prototype, "showCopy", 2);
__decorateClass([
  property({ type: Boolean, reflect: true })
...
], OmniAssistChatCircle.prototype, "genAiModels", 2);
__decorateClass([
  property({ attribute: false })
], OmniAssistChatCircle.prototype, "selectedGenAiModel", 2);
__decorateClass([
  query("omni-textarea")
