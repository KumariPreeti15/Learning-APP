import { OmniElement } from '../../omni-element.js';
import type { ChatHistoryItem, GenAiModel } from './omni-assist.types.js';
import type { OmniTextarea } from '../omni-textarea/omni-textarea.js';
import '../omni-dot-loader/omni-dot-loader.js';
import '../omni-markdown/omni-markdown.js';
import './omni-assist-feedback-controls.js';
import '../omni-tooltip/omni-tooltip.js';
import '../omni-icon/omni-icon.js';
/**
 * To include within your project `import 'omni-ui/omni-assist-chat-circle.js'`
 *
 * @tagname omni-assist-chat-circle
 * @event {ChatSuggestion} submit-chat - fired when user submits chat through click or Enter
 * @event {Feedback} feedback - fired when user upvotes or downvotes chat response
 * @event {GenAiModel} gen-ai-model-change - fired when user changes the selected GenAiModel
 */
export declare class OmniAssistChatCircle extends OmniElement {
    #private;
    static get styles(): import("lit").CSSResult[];
    /** show the three dot loader and disable the input field */
    isLoading: boolean;
    /** indicate that streaming is occurring and disable the input field */
    isStreaming: boolean;
    /** array of chat history items to display */
    history: ChatHistoryItem[];
    /** address to user profile image */
    userImage: string;
    isOpen: boolean;
    cornerGravity: boolean;
    expanded: boolean;
    /** display the feedback response controls */
    unhide: boolean;
    showCopy: boolean;
    /** @internal */
    dragging: boolean;
    offsetLeft: number;
    genAiModels: GenAiModel[];
    selectedGenAiModel: GenAiModel;
    /** @internal */
    textarea: OmniTextarea;
    connectedCallback(): Promise<void>;
    willUpdate(changed: Map<string, unknown>): void;
    updated(changed: Map<string, unknown>): void;
    scrollDown(): Promise<void>;
    disconnectedCallback(): void;
    handleChat(): Promise<void>;
    render(): import("lit-html").TemplateResult<1>;
}
declare global {
    interface Window {
        AN: {
            sso: {
                user: {
                    userImage: string;
                    fullName: string;
                    firstName: string;
                };
            };
        };
    }
    interface HTMLElementTagNameMap {
        'omni-assist-chat-circle': OmniAssistChatCircle;
    }
}
export type { ChatHistoryItem, GenAiModel };
//# sourceMappingURL=omni-assist-chat-circle.d.ts.map