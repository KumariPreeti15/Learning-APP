...
class OmniOptions extends OmniElement {
  constructor() {
    ...
    this.#optionListItem = (option, index) => {
      ...
      <div
        ...
        ${typeof option === "object" && option?.postTemplate ? html`<div class="post-template">${option.postTemplate}</div>` : this.optionsPostTemplate ? html`<div class="post-template">${this.optionsPostTemplate(option)}</div>` : nothing}
      </div>
    `;
  ...
  async firstUpdated() {
    if (!this.options.some((option) => typeof option === "object" && option?.attributes)) return;
